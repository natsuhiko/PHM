        ...
        for(j=0; j<ncol; j++){
            while((c=gzgetc(f)) != EOF){
                if(c=='\n' || c=='\t' || c=='\0'){
                    cell[k] = '\0';
                    if(type[j]=='B'){
                        sscanf(cell, "%lf", bf+i+nrow*nexp[j]);
                        bf[i+nrow*nexp[j]] = exp(bf[i+nrow*nexp[j]]);
                        if(isnan(bf[i+nrow*nexp[j]])>0){
                            bf[i+nrow*nexp[j]] = 0.0;
                            fprintf(stderr, "NaN is found at (%d, %d) element! (replaced by 0.0)\n", i, j);
                        }
                    }else if(type[j]=='O'){
                        sscanf(cell, "%lf", &dcell);
                        X[i*3+(2*P)*ldx] = X[i*3+1+(2*P)*ldx] = X[i*3+2+(2*P)*ldx] = dcell;
                    }else if(type[j]=='N'){
                        sscanf(cell, "%lf", &dcell);
                        ...
                        X[1    *ldx + i*3+0] = apeakdis;
                        X[(P+1)*ldx + i*3+1] = apeakdis;
                        X[(P+1)*ldx + i*3+2] = apeakdis;
                        
                        
                        for(k=0; k<4; k++){
                            Covs[(2+k)*ldx + i*3+0] = rk1(apeakdis, xk6[k]);
                            Covs[(8+k)*ldx + i*3+1] = rk1(apeakdis, xk6[k]);
                            Covs[(8+k)*ldx + i*3+2] = rk1(apeakdis, xk6[k]);
                        }
