...
void emATACMultinom(double* bf, double* X, long N, long P, long LDXT, double* Z, double* z1, double* beta, long nthreads){
    ...
    fprintf(stderr, "Data split into %ld threads with eath %ld peak pairs\n", nthreads, npp);
    ...
    for(itr=0; itr<10001; itr++){
        ...
        if(fabs(lkhd-lkhd0)<1e-7){break;}else{lkhd0=lkhd; fprintf(stderr, "[%ld] lkhd=%lf\n", itr, lkhd);}
...
int main(int argc, char** argv){
    ...
    long nthreads = 1;
    for(k=0; k<argc-1; k++){if(strcmp(argv[k],"--nthreads")==0){nthreads = (long)atoi(argv[k+1]);}}
    
    long nrow=0, ncol=0;
    for(k=0; k<argc-1; k++){if(strcmp(argv[k],"--dim")==0){
        sscanf(argv[k+1], "%ld,%ld", &nrow, &ncol);
        break;
    }}
    
    if(nrow==0 && ncol==0){// GZ files
        gzFile f = gzopen(argv[1], "rb6f");
        dim(f, &nrow, &ncol, 0);
        gzclose(f);
    }
    
    fprintf(stderr, "Data: %ld x %ld\n", nrow, ncol-2);
    ...
    fprintf(stderr, "Init beta:"); printV2(beta, P); fprintf(stderr, "\n");
    ...
    for(k=0; k<argc-1; k++){if(strcmp(argv[k],"--output-beta")==0){
    ...
    for(k=0; k<argc-1; k++){if(strcmp(argv[k],"--output")==0){
        long kk, binf=0;
